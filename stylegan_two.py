...
def nImage(n):  #changed
    return np.random.uniform(0.0, 1.0, size = [n, to_shape[0], to_shape[1], 1]).astype('float32')
...
def g_block(inp, istyle, inoise, fil, u = True):
    ...
    if u:
        ...
        out = Lambda(upsample, output_shape=[None, inp.shape[1] * 2, inp.shape[2] * 2, None])(inp)
    else:
...
def to_rgb(inp, style): #changed
    size = inp.shape[1]
    x = Conv2DMod(3, 1, kernel_initializer = VarianceScaling(200/size), demod = False)([inp, style])
    return Lambda(upsample_to_size, output_shape=[None, to_shape[0], to_shape[1], None])(x)

def from_rgb(inp, conc = None): #changed
    fil = int(to_shape[0] * 4 / inp.shape[1])
...
class GAN(object):
    ...
    def discriminator(self):
        ...
        inp = Input(shape = [to_shape[0], to_shape[1], 3]) #changed
    ...
    def generator(self):
        ...
        inp_noise = Input([to_shape[0], to_shape[1], 1]) #changed
...
class StyleGAN(object):
    ...
    def __init__(self, steps = 1, lr = 0.0001, decay = 0.00001, silent = True):
        ...
        self.im = dataGenerator(directory, flip = True) #changed
    ...
    def train(self):
        ...
        images, to_shape = self.im.get_batch(BATCH_SIZE)

        a, b, c, d = self.train_step(images.astype('float32'), style, nImage(BATCH_SIZE), apply_gradient_penalty, apply_path_penalty)
    ...
    def evaluate(self, num = 0, trunc = 1.0):
        ...
        generated_images = self.GAN.GMA.predict(latent + [nImage(64, (512, 900)), trunc], batch_size = BATCH_SIZE)
